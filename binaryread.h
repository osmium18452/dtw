...
short ReadShort()
{
	...
	unsigned char* machine = (unsigned char*)&data; //
	for( int i=1; i>=0; i-- )
	{
...
int ReadInt()
{
	...
	unsigned char* machine = (unsigned char*)&data; // machine code, 4 bytes.
	for( int i=3; i>=0; i-- )
	{
		InFile.read( &cache[i], 1 );
	}
...
float ReadFloat()
{
	float data = 0.0f;
	char *cache = new char [4];
	unsigned char *machine = ( unsigned char* )&data; // machine code
	for( int i=3; i>=0; i-- )
	{
		InFile.read( &cache[i], 1 );	
	}
